  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+6912 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 22352;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+7040 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 22488;
    ...
    k = (int32_t *) (l1_buffer + 22352);
    lambda = (int32_t *) (l1_buffer + 22488);
