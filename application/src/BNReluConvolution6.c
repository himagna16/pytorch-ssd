  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+20736 + 192*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 26904;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+20928 + 192*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 27104;
    ...
    k = (int32_t *) (l1_buffer + 26904);
    lambda = (int32_t *) (l1_buffer + 27104);
