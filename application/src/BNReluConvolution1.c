...
void BNReluConvolution1(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 33 : 33;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 25 : 41;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 33, 41, 16, 64, 16,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 29344);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+3456 + 96*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 32808;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+3552 + 96*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 32912;
    ...
    k = (int32_t *) (l1_buffer + 32808);
    lambda = (int32_t *) (l1_buffer + 32912);
    W = (uint8_t *) (l1_buffer + 29344);
    y = (uint8_t *) (l1_buffer + 21656);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 16, 20, 24, 32, 24, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 21656);
