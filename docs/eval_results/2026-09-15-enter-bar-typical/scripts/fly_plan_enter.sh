...
#   (vis_enter 0.75, confirm_frames 3, vis_exit 0.45).  Its CORE matrix is also
#   hard-coded and `--only` is a filter on that matrix, so it cannot name the
#   tp* scenes at all.  Rather than edit the harness -- the brief forbids
#   touching tools/ -- the fly() body below is a faithful copy of the harness's
#   own fly(), exactly as docs/eval_results/2026-09-14-exit-bar/scripts/
#   fly_plan_exit.sh did, with the same two deliberate changes:
#     - the order comes from the plan file, not from a nested cell x repeat loop,
#       so arms can be interleaved and arm decorrelated from time;
#     - load average is sampled immediately before and after every flight;
#   plus the one thing this experiment needs:
#     - THE ENTER BAR IS A PARAMETER, passed to the repo's own follow_person.py
#       as --vis-enter.  --vis-exit 0.45 and --confirm-frames 3 are ALSO passed
#       explicitly, at their default values, so both arms are constructed
#       identically and the exit bar / confirmation count are pinned on the
#       command line rather than inherited.  For the 0.75 arm this command line
#       is behaviourally identical to the harness's (it passes the same values
#       the defaults give), which is what makes the 0.75 arm here comparable to
#       last night's no-flag flights.
#   run_acceptance2.sh, follow_person.py, camera_model.py, build_scene.py,
#   scoreboard.py and patch_crazysim.py are NOT modified.  No scene is built or
#   rebuilt here: the four tp* scenes and the two originals are the artefacts
#   2026-09-15-typical-person built last night, reused in place.
#
...
#
# Usage: fly_plan_enter.sh PLAN.tsv OUT_DIR SCENE_ROOT [START_ORDER] [STOP_EPOCH]
set -uo pipefail
...
# fly <vis_enter> <cell> <scene> <class> <backend> <camera> <speed> <dur> <rep> <att> <order>
fly() {
  local venter=$1 cid=$2 scene=$3 cls=$4 backend=$5 camera=$6 speed=$7 dur=$8 rep=$9 att=${10} ord=${11}
  local vexit=0.45 cframes=3
  local want=0                      # every cell here is flown on the matte floor
  local sdir="$SCENE_ROOT/$scene"
  local run="$OUT/runs/e${venter}__${cid}__r${rep}a${att}"
  rm -rf "$run"; mkdir -p "$run"

  if [ ! -f "$sdir/scene.xml" ]; then
    log "  [e$venter $cid r$rep a$att] SCENE MISSING: $sdir/scene.xml"
  ...
  if [ "$(echo "$refl $want" | awk '{print ($1==$2)?"ok":"no"}')" != ok ] \
    ...
    log "  [e$venter $cid r$rep a$att] ABORT: floor is xml='$refl' manifest='$man_refl', wanted $want"
  ...
  shasum -a 256 "$sdir/scene.xml" | awk '{print $1}' > "$run/scene_xml_sha256.txt"
...
PYEOF
  ...
  if docker ps -a --format '{{.Names}}' | grep -qx crazysim-mac \
    ...
    log "  [e$venter $cid r$rep a$att] a simulator is already running; tearing it down first"
    pkill -f "crazysim_mujoco/crazysim.py" 2>/dev/null
  ...
  if [ "$camera" = clean ]; then
    ...
    # two arms at the same repeat see the SAME himax noise draw.
  ...
  if ! grep -q "firmware connected" "$run/sim.log" 2>/dev/null; then
    log "  [e$venter $cid r$rep a$att] SIM DID NOT START"
...
log "enter-bar suite starting: plan=$PLAN out=$OUT scenes=$SCENE_ROOT start=$START stop_epoch=$STOP_EPOCH"
T0=$(date +%s)
NPLAN=$(grep -vc '^#' "$PLAN")

while IFS=$'\t' read -r ord venter cid scene cls backend camera speed dur rep block; do
  [ -z "$ord" ] && continue
  ...
  for att in 1 2; do
    ...
    t_start=$(date +%s)
    log "flight $ord/$NPLAN  vis_enter $venter  $cid  r$rep a$att  (block $block, seed $((1000+rep)), load1 $lb1)"
    take_lock
    fly "$venter" "$cid" "$scene" "$cls" "$backend" "$camera" "$speed" "$dur" "$rep" "$att" "$ord"
    rc=$?
    drop_lock
    t_end=$(date +%s)
    la1=$(load1); la5=$(load5); la15=$(load15)
    run="$OUT/runs/e${venter}__${cid}__r${rep}a${att}"
...
" "$run/summary.json" "$venter" 0.45 3 2>&1)"
...
p = os.path.join(run, "scene_xml_sha256.txt")
if os.path.exists(p):
    rec["scene_xml_sha256"] = open(p).read().strip()
