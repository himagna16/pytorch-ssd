...
MODELS = ["champion", "confuser", "confuser_qathn", "confuser_qathn3_ep2"]
FLOAT_CONFUSER_REF = {"champion": 0.239, "confuser": 0.083, "confuser_qathn": 0.122, "confuser_qathn3_ep2": 0.084}
Z = 1.959963984540054
BOOT = 2000
    ...
    for m in MODELS:
        ...
        for cname, li in chans.items():
            ...
            if cname.startswith("runtime"):
                ...
                # per-image CSV
                with open(OUT / f"per_image4_{m}.csv", "w", newline="") as f:
                    w = csv.writer(f)
            ...
            mr[cname] = blk
        res[m] = mr
    json.dump(res, open(OUT / "results_4.json", "w"), indent=1)
