...
1. **Stream check.** Run `python pytorch_ssd/tools/crazysim_macos/cpx_grab.py --n 5 --every 1 --out ~/drone_frames/<date>/check`.
   ...
   **every** `cpx_grab.py` command from here on. (`--bayer` averages each 2x2
   ...
   `examples/other/wifi-img-streamer/opencv-viewer.py -n 192.168.4.1 -p 5000`
   in `aideck-gap8-examples`. It always converts raw frames to color, so use it
   only to see that the stream works. Do not score its `--save` images.
2. **Mirror check (must pass).** Record two 10 s clips into their own folder,
   ...
   **2.5 m, +25° (drone's RIGHT)**:
   ...
   ```bash
   python pytorch_ssd/tools/crazysim_macos/cpx_grab.py --seconds 10 --every 1 \
     --dist 2.5 --bearing -25 --vis 1 --subject p01 --light room --out ~/drone_frames/<date>/mirror
   python pytorch_ssd/tools/crazysim_macos/cpx_grab.py --seconds 10 --every 1 \
     --dist 2.5 --bearing 25 --vis 1 --subject p01 --light room --out ~/drone_frames/<date>/mirror
   ```

   Then score exactly that folder:
   `score_real_frames.py ~/drone_frames/<date>/mirror` (section 6). The MIRROR
...
```bash
python pytorch_ssd/tools/crazysim_macos/cpx_grab.py --seconds 30 --every 1 \
...
One 30 s clip is roughly a few hundred frames at about 50 KB each.

## 6. Scoring

```bash
~/Downloads/drone/trainenv/bin/python pytorch_ssd/tools/real_frames/score_real_frames.py \
...
Synthetic check (Sep 10, 660 rendered sim frames): mirror PASS, 0 false
